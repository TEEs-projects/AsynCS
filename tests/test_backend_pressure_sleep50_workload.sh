...
crate="$root/workloads/backend_pressure_sleep50"
...
built_wasm="$tmp/target/wasm32-wasip1/release/asyncs-backend-pressure-sleep50.wasm"
versioned_wasm="$crate/asyncs-backend-pressure-sleep50.wasm"
...
if grep -Fq 'clock_time_get' "$tmp/imports.txt"; then
  echo "sleep50 workload should use poll_oneoff without an extra WASI clock query" >&2
  exit 1
fi
...
assert contract["workload_id"] == "asyncs-wasm-sleep-50ms", contract
assert contract["workload_logic_hash"] == "rust-std-thread-sleep-50ms-v1", contract
assert contract["workload_kind"] == "wasm_sleep", contract
assert contract["workload_sleep_ms"] == 50, contract
...
grep -Fq 'const SLEEP_MS: u64 = 50;' "$crate/src/main.rs"
...
if grep -Fq 'println!' "$crate/src/main.rs"; then
  echo "sleep50 workload output must use the trusted WAMR host interface" >&2
  exit 1
fi

echo "PASS: AsynCS backend-pressure sleep50 workload is reproducible and releases CPU through the existing WASI/SGX sleep bridge"
